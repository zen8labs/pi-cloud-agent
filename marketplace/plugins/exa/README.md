...
3. After sign-in, `EXA_ACCESS_TOKEN` is stored encrypted and injected as `Authorization: Bearer …` at provision time.

Paste fallback: configure `EXA_ACCESS_TOKEN` manually with an API key from https://dashboard.exa.ai/api-keys (Exa accepts a plain API key as Bearer).
