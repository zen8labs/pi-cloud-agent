...
- **A lifecycle-shaped sandbox contract.** `resolveImage`, `create`, `resume`, `suspend`, `deleteWorkspace`, and `stop`. The sandbox remains outbound-only; persistence is a provider concern, not an agent server. → [docs/adding-a-sandbox-provider.md](docs/adding-a-sandbox-provider.md)
...
- [Pierre Diffs](https://github.com/pierrecomputer/pierre) - the diff renderer (`@pierre/diffs`).
