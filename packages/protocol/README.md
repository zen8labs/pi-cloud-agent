...
The contracts. Types, schemas, and the three interfaces that make this core extensible.
...
- **`profile.ts` deliberately erases the config type.** `ProfileDefinition<Config>` is what a profile author writes; `Profile` is what the controller sees. That asymmetry is the mechanism that stops the controller branching on a profile-specific field. Do not "simplify" it away.
