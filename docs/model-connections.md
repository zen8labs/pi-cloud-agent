...
Codex sign-in uses Pi's device-code flow. The controller picks `device_code` itself; the browser never submits prompt input. A flow is bound to the authenticated user, streamed over SSE, and can be cancelled with `DELETE`. Starting another sign-in for the same user supersedes the previous attempt. The flow expires shortly after the provider's device-code lifetime. The manager is in-memory: suitable for a single controller instance, not restart-safe or horizontally scalable. Before multiple replicas, move flow state to shared durable storage. Never put access or refresh tokens in query parameters, browser local storage, logs, or event history returned to the UI.
