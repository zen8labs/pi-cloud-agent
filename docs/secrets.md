...
The optional per-repository setup script in Settings runs in that same untrusted checkout. It can use the forge credential for private dependencies, but the runtime withholds model credentials, the run callback token, and plugin configuration before invoking it. Do not put any additional credentials in the script or its output.
