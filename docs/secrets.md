...
- The runtime must not persist model OAuth credentials in the parked session workspace. Pi may use a run-scoped temporary auth file during a turn, but the file is removed before the sandbox can be suspended.
